...
esp_err_t display_init(void)
{
    ...
    /* LVGL timer task — single task owns lv_timer_handler(). Pinned to core 1
     * so it doesn't contend with WiFi/LWIP on core 0 (PRO_CPU). */
    BaseType_t r = xTaskCreatePinnedToCore(lvgl_task, "lvgl", 6144, NULL, 5, NULL, 1);
    if (r != pdPASS) {
