...
/* sensor_hub uniform-driver handle. Valid only after bmx280_init() succeeds
 * (caps are derived from the detected chip type). Register with sensor_hub. */
...
/* Return the I2C bus handle — used by the bsec and scd4x components to add
 * their own device. Valid whenever bmx280_init() managed to create the I2C
 * bus, even if no BMx280/BME680 chip was found on it (bmx280_init() returns
 * ESP_ERR_NOT_FOUND in that case but leaves the bus up for other sensors).
 * Returns NULL only if bus creation itself failed. */
