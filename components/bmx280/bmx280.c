...
esp_err_t bmx280_init(void)
{
    ...
    if (!found) {
        ESP_LOGW(TAG, "no BMx280/BME680 at 0x76 or 0x77 — leaving I2C bus up "
                 "for other sensors (e.g. scd4x)");
...
static esp_err_t bmx280_drv_read(sensor_driver_t *self, sensor_reading_t *out)
{
    (void)self;
    bmx280_data_t d;
    if (bmx280_read(&d) != ESP_OK) return ESP_FAIL;
    ...
    /* BME680 without BSEC exposes a gas-resistance-derived CO2-equivalent. */
    if (s_type == BMX280_TYPE_BME680) {
        float co2eq;
        if (bmx280_co2eq_avg(&co2eq) == ESP_OK) {
            out->co2          = co2eq;
...
static sensor_driver_t s_hub_driver = {
    .name = "bmx280",
    .read = bmx280_drv_read,
};

sensor_driver_t *bmx280_driver(void)
{
    sensor_caps_t caps = 0;
    if (s_type != BMX280_TYPE_NONE)
        caps |= SENSOR_CAP_TEMPERATURE | SENSOR_CAP_PRESSURE;
    if (s_type == BMX280_TYPE_BME280 || s_type == BMX280_TYPE_BME680)
        caps |= SENSOR_CAP_HUMIDITY;
    if (s_type == BMX280_TYPE_BME680)
        caps |= SENSOR_CAP_GAS_RESISTANCE | SENSOR_CAP_CO2;
    s_hub_driver.caps = caps;
    return &s_hub_driver;
}