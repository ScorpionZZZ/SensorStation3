...
static char s_client_id[24];   /* "ss3-tb-aabbccddeeff" — must differ from
                                 * domoticz_mqtt's/ha_mqtt's client IDs or the
...
static void ensure_client_id(void)
{
    ...
    snprintf(s_client_id, sizeof(s_client_id), "ss3-tb-%02x%02x%02x%02x%02x%02x",
