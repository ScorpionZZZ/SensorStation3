...
/* Preload the 24-hour ring buffer from an external source (e.g. a backend's
 * REST API at boot), so the main-screen charts don't start empty. slots[]
 * must be oldest-first, one entry per 10-minute bucket, with the last entry
 * being the most recent (matching what sensor_history_get_24h() itself
 * returns). Individual slots may have valid == false to represent gaps.
 * count is clamped to SENSOR_HISTORY_24H_SLOTS, keeping the most recent
 * entries if more are passed. Meant to be called once, early — calling it
 * after the local sampling task has already committed slots discards them. */
void sensor_history_seed_24h(const history_slot_t *slots, int count);