...
void sensor_history_seed_24h(const history_slot_t *slots, int count)
{
    ...
    if (count > HIST_24H_SLOTS) {
        slots += (count - HIST_24H_SLOTS);   /* keep only the most recent ones */
        count  = HIST_24H_SLOTS;
    }

    xSemaphoreTake(s_mutex, portMAX_DELAY);
    for (int i = 0; i < count; i++) s_hist[i] = slots[i];
    s_hist_head  = count % HIST_24H_SLOTS;
    s_hist_count = count;
    xSemaphoreGive(s_mutex);
    ESP_LOGI(TAG, "seeded %d/%d history slots", count, HIST_24H_SLOTS);
}