...
{
    strncpy(out, fw_url, out_len - 1);
    out[out_len - 1] = '\0';
    char *dot_bin = strstr(out, ".bin");
...
esp_err_t ota_manager_check_now(const char *check_url)
{
    ...
    if (s_check_state == OTA_CHECK_CHECKING) return ESP_ERR_INVALID_STATE;

    strncpy(s_check_url, check_url, sizeof(s_check_url) - 1);
    s_check_url[sizeof(s_check_url) - 1] = '\0';

    BaseType_t ok = xTaskCreate(check_task, "ota_chk", 4096, NULL, 3, NULL);
    return (ok == pdPASS) ? ESP_OK : ESP_ERR_NO_MEM;
}
