...
/* Derive a JSON manifest check URL from a firmware .bin URL by replacing the
 * trailing ".bin" with ".json"; if fw_url has no trailing ".bin", out is a
 * plain copy of fw_url. out must be at least out_len bytes. */
void ota_manager_derive_check_url(const char *fw_url, char *out, size_t out_len);

/* Trigger an immediate one-off check against check_url, outside the
 * scheduled timer cadence — use when the user opens the update-check UI or
 * changes the firmware URL. Returns ESP_ERR_INVALID_ARG if check_url is
 * empty, ESP_ERR_INVALID_STATE if a check is already in progress. */
esp_err_t ota_manager_check_now(const char *check_url);
