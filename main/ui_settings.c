...
static void hist_source_cb(lv_event_t *e)
{
    lv_obj_t *dd = lv_event_get_target(e);
    uint16_t sel = lv_dropdown_get_selected(dd);
    nvs_settings_set_hist_source((uint8_t)sel);
    ESP_LOGI(TAG, "24h history chart source: %u", (unsigned)sel);
}
...
static void units_enable_cb(lv_event_t *e)
{
    lv_obj_t *cb = lv_event_get_target(e);
    bool use_fahrenheit = lv_obj_has_state(cb, LV_STATE_CHECKED);
    nvs_settings_set_use_fahrenheit(use_fahrenheit);
    ui_main_set_use_fahrenheit(use_fahrenheit);
    ESP_LOGI(TAG, "Display units: %s", use_fahrenheit ? "Fahrenheit" : "Celsius");
}
...
void ui_settings_create(void)
{
    ...
    const int units_extra_y = 56;

    lv_obj_t *units_hdr = lv_label_create(s_screen);
    lv_label_set_text(units_hdr, "Display Units:");
    lv_obj_set_style_text_color(units_hdr, lv_color_white(), 0);
    lv_obj_align(units_hdr, LV_ALIGN_TOP_LEFT, 10, 272);

    bool use_fahrenheit = false;
    nvs_settings_get_use_fahrenheit(&use_fahrenheit);

    lv_obj_t *units_cb = lv_checkbox_create(s_screen);
    lv_checkbox_set_text(units_cb, "Use Fahrenheit (\xc2\xb0" "F)");
    lv_obj_set_style_text_color(units_cb, lv_color_white(), 0);
    lv_obj_align(units_cb, LV_ALIGN_TOP_LEFT, 10, 290);
    if (use_fahrenheit)
        lv_obj_add_state(units_cb, LV_STATE_CHECKED);
    lv_obj_add_event_cb(units_cb, units_enable_cb, LV_EVENT_VALUE_CHANGED, NULL);
    ...
    lv_obj_align(mqtt_hdr, LV_ALIGN_TOP_LEFT, 10, 272 + units_extra_y);

    s_mqtt_status = lv_label_create(s_screen);
    lv_obj_set_width(s_mqtt_status, 220);
    lv_obj_align(s_mqtt_status, LV_ALIGN_TOP_LEFT, 10, 290 + units_extra_y);
    refresh_mqtt_status();
    ...
    lv_obj_align(mqtt_cb, LV_ALIGN_TOP_LEFT, 10, 310 + units_extra_y);
    if (mqtt_enabled)
    ...
    lv_obj_align(mqtt_btn, LV_ALIGN_TOP_MID, 0, 340 + units_extra_y);
    ...
    lv_obj_align(dz_hdr, LV_ALIGN_TOP_LEFT, 10, 388 + units_extra_y);

    s_dz_status = lv_label_create(s_screen);
    lv_obj_set_width(s_dz_status, 220);
    lv_obj_align(s_dz_status, LV_ALIGN_TOP_LEFT, 10, 406 + units_extra_y);
    refresh_dz_status();
    ...
    lv_obj_align(dz_cb, LV_ALIGN_TOP_LEFT, 10, 424 + units_extra_y);
    if (dz_enabled)
    ...
    lv_obj_align(dz_btn, LV_ALIGN_TOP_MID, 0, 448 + units_extra_y);
    ...
    lv_obj_align(ha_hdr, LV_ALIGN_TOP_LEFT, 10, 496 + units_extra_y);

    s_ha_status = lv_label_create(s_screen);
    lv_obj_set_width(s_ha_status, 220);
    lv_obj_align(s_ha_status, LV_ALIGN_TOP_LEFT, 10, 514 + units_extra_y);
    refresh_ha_status();
    ...
    lv_obj_align(ha_cb, LV_ALIGN_TOP_LEFT, 10, 532 + units_extra_y);
    if (ha_enabled)
    ...
    lv_obj_align(ha_btn, LV_ALIGN_TOP_MID, 0, 556 + units_extra_y);
    ...
    /* ── 24h History Chart section ────────────────────────────────────── */
    const int hist_extra_y = 56;

    lv_obj_t *hist_hdr = lv_label_create(s_screen);
    lv_label_set_text(hist_hdr, "24h History Chart:");
    lv_obj_set_style_text_color(hist_hdr, lv_color_white(), 0);
    lv_obj_align(hist_hdr, LV_ALIGN_TOP_LEFT, 10, 496 + units_extra_y + ha_extra_y);

    uint8_t hist_source = NVS_SETTINGS_HIST_SOURCE_NONE;
    nvs_settings_get_hist_source(&hist_source);

    lv_obj_t *hist_dd = lv_dropdown_create(s_screen);
    lv_dropdown_set_options(hist_dd, "Off\nThingsBoard\nDomoticz\nHome Assistant");
    lv_dropdown_set_selected(hist_dd, hist_source);
    lv_obj_set_size(hist_dd, 220, 36);
    lv_obj_align(hist_dd, LV_ALIGN_TOP_MID, 0, 514 + units_extra_y + ha_extra_y);
    lv_obj_add_event_cb(hist_dd, hist_source_cb, LV_EVENT_VALUE_CHANGED, NULL);
    ...
    if (bmx280_get_type() == BMX280_TYPE_BME680) {
        ...
        lv_obj_align(toff_hdr, LV_ALIGN_TOP_LEFT, 10, 496 + units_extra_y + ha_extra_y + hist_extra_y);
        ...
        lv_obj_align(toff_row, LV_ALIGN_TOP_MID, 0, 514 + units_extra_y + ha_extra_y + hist_extra_y);
    ...
    lv_obj_align(ota_btn, LV_ALIGN_TOP_MID, 0, 496 + units_extra_y + ha_extra_y + hist_extra_y + bme680_extra_y);
    ...
    /* ── Device Info section ─────────────────────────────────────────── */
    int info_y = 542 + units_extra_y + ha_extra_y + hist_extra_y + bme680_extra_y;
