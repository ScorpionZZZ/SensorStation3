...
#define KEY_USE_FAHRENHEIT "use_fahrenheit"
...
esp_err_t nvs_settings_init(void)
{
    ...
    char ota_url[NVS_SETTINGS_OTA_URL_LEN + 1];
    size_t ota_url_len = sizeof(ota_url);
    if (nvs_get_str(h, KEY_OTA_URL, ota_url, &ota_url_len) == ESP_ERR_NVS_NOT_FOUND) {
        nvs_set_str(h, KEY_OTA_URL, NVS_SETTINGS_OTA_URL_DEFAULT);
        nvs_commit(h);
    }
...
esp_err_t nvs_settings_get_ota_url(char *out, size_t len)
{
    ...
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) {
        strncpy(out, NVS_SETTINGS_OTA_URL_DEFAULT, len - 1);
        out[len - 1] = '\0';
        return ESP_OK;
    }
    return ret;
...
esp_err_t nvs_settings_get_ha_uri(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_HA_URI, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_ha_uri(const char *uri)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_HA_URI, uri);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_ha_enabled(bool *out)
{
    ...
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    uint8_t v = 0;
    esp_err_t ret = nvs_get_u8(h, KEY_HA_ENABLED, &v);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { *out = false; return ESP_OK; }
    *out = (v != 0);
    return ret;
...
esp_err_t nvs_settings_set_ha_enabled(bool enabled)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_u8(h, KEY_HA_ENABLED, enabled ? 1 : 0);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_ha_user(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_HA_USER, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_ha_user(const char *user)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_HA_USER, user);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_ha_pass(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_HA_PASS, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_ha_pass(const char *pass)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_HA_PASS, pass);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_use_fahrenheit(bool *out)
{
    ...
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    uint8_t v = 0;
    esp_err_t ret = nvs_get_u8(h, KEY_USE_FAHRENHEIT, &v);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { *out = false; return ESP_OK; }
    *out = (v != 0);
    return ret;
...
esp_err_t nvs_settings_set_use_fahrenheit(bool enabled)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_u8(h, KEY_USE_FAHRENHEIT, enabled ? 1 : 0);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_hist_source(uint8_t *out)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    uint8_t v = NVS_SETTINGS_HIST_SOURCE_NONE;
    esp_err_t ret = nvs_get_u8(h, KEY_HIST_SOURCE, &v);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { *out = NVS_SETTINGS_HIST_SOURCE_NONE; return ESP_OK; }
    *out = v;
    return ret;
}
...
esp_err_t nvs_settings_set_hist_source(uint8_t source)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_u8(h, KEY_HIST_SOURCE, source);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_dz_http_url(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_DZ_HTTP_URL, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_dz_http_url(const char *url)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_DZ_HTTP_URL, url);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_dz_http_user(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_DZ_HTTP_USER, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_dz_http_user(const char *user)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_DZ_HTTP_USER, user);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_dz_http_pass(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_DZ_HTTP_PASS, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_dz_http_pass(const char *pass)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_DZ_HTTP_PASS, pass);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_tb_rest_url(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_TB_REST_URL, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_tb_rest_url(const char *url)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_TB_REST_URL, url);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_tb_rest_user(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_TB_REST_USER, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_tb_rest_user(const char *user)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_TB_REST_USER, user);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_tb_rest_pass(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_TB_REST_PASS, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_tb_rest_pass(const char *pass)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_TB_REST_PASS, pass);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_tb_device_id(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_TB_DEVICE_ID, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_tb_device_id(const char *id)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_TB_DEVICE_ID, id);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_ha_http_url(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_HA_HTTP_URL, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_ha_http_url(const char *url)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_HA_HTTP_URL, url);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_ha_http_token(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_HA_HTTP_TOKEN, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_ha_http_token(const char *token)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_HA_HTTP_TOKEN, token);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_ha_temp_entity(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_HA_TEMP_ENTITY, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_ha_temp_entity(const char *entity_id)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_HA_TEMP_ENTITY, entity_id);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}
...
esp_err_t nvs_settings_get_ha_hum_entity(char *out, size_t len)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READONLY, &h), TAG, "open");
    esp_err_t ret = nvs_get_str(h, KEY_HA_HUM_ENTITY, out, &len);
    nvs_close(h);
    if (ret == ESP_ERR_NVS_NOT_FOUND) { out[0] = '\0'; return ESP_OK; }
    return ret;
}
...
esp_err_t nvs_settings_set_ha_hum_entity(const char *entity_id)
{
    nvs_handle_t h;
    ESP_RETURN_ON_ERROR(nvs_open(NVS_NS, NVS_READWRITE, &h), TAG, "open");
    esp_err_t ret = nvs_set_str(h, KEY_HA_HUM_ENTITY, entity_id);
    if (ret == ESP_OK) ret = nvs_commit(h);
    nvs_close(h);
    return ret;
}