...
static void refresh_info_line(void)
{
    ...
    case OTA_CHECK_UP_TO_DATE: {
        ...
        ota_manager_get_server_version(sv, sizeof(sv));
        if (sv[0])
            snprintf(buf, sizeof(buf), "Up to date (latest: v%s)", sv);
        else
            snprintf(buf, sizeof(buf), "Firmware is up to date");
        lv_label_set_text(s_info_lbl, buf);
        lv_obj_set_style_text_color(s_info_lbl, lv_palette_main(LV_PALETTE_GREEN), 0);
        break;
    }
    case OTA_CHECK_FAILED:
        lv_label_set_text(s_info_lbl, "Latest version: N/A");
...
/* Kick off an immediate version check against url's derived .json manifest
 * and reflect it right away (the 2 s info_timer_cb picks up further state
 * changes: checking -> available/up-to-date/failed). */
static void trigger_check(const char *url)
{
    if (!url || url[0] == '\0') return;
    char check_url[NVS_SETTINGS_OTA_URL_LEN + 1];
    ota_manager_derive_check_url(url, check_url, sizeof(check_url));
    ota_manager_check_now(check_url);
    refresh_info_line();
}
...
void ui_ota_show(void)
{
    ...
    lv_obj_add_event_cb(s_kbd, kbd_cancel_cb, LV_EVENT_CANCEL, NULL);
}
