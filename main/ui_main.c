...
/* true when a BMx280 or SCD4x sensor was detected at boot; false hides the
 * sensor panels behind an error card and skips all sensor_history_* calls
 * (its mutex is never created when sensor_history_init() didn't run). */
static bool s_sensor_present;
...
/* true when the user has selected Fahrenheit in Settings; false = Celsius
 * (default). Only affects this screen's temperature/dew-point display —
 * MQTT telemetry always publishes °C. */
static bool s_use_fahrenheit;

/* temperature card */
static lv_obj_t         *s_lbl_temp;
static lv_obj_t         *s_lbl_temp_unit;
...
static lv_obj_t *s_lbl_dew_unit;
...
/* Convert a Celsius reading to the display unit currently selected. */
static float temp_to_display(float c)
{
    return s_use_fahrenheit ? (c * 9.0f / 5.0f + 32.0f) : c;
}

/* Convert a Celsius *delta* (trend badge) — no +32 offset, since it's a
 * difference, not an absolute value. */
static float temp_delta_to_display(float dc)
{
    return s_use_fahrenheit ? (dc * 9.0f / 5.0f) : dc;
}

void ui_main_set_use_fahrenheit(bool use_fahrenheit)
{
    s_use_fahrenheit = use_fahrenheit;
    const char *unit = use_fahrenheit ? "\xc2\xb0\x46" /* °F */
                                       : "\xc2\xb0\x43" /* °C */;
    if (s_lbl_temp_unit) lv_label_set_text(s_lbl_temp_unit, unit);
    if (s_lbl_dew_unit)  lv_label_set_text(s_lbl_dew_unit, unit);
}

/* Field availability comes from the hub's aggregate capability set — fixed for
 * the session once the sensors are probed and registered. */
static bool has_humidity(void)
{
    return sensor_hub_has(SENSOR_CAP_HUMIDITY);
}

static bool has_pressure(void)
{
    return sensor_hub_has(SENSOR_CAP_PRESSURE);
}

static bool has_co2(void)
{
    return sensor_hub_has(SENSOR_CAP_CO2);
}
...
static void tick_timer_cb(lv_timer_t *t)
{
    ...
    if (s_sensor_present) {
        sensor_reading_t s;
        bool valid = sensor_history_get_snapshot(&s) &&
                     (s.valid & SENSOR_CAP_TEMPERATURE);
        if (valid) {
            char buf[16];

            snprintf(buf, sizeof(buf), "%.1f", temp_to_display(s.temperature));
            lv_label_set_text(s_lbl_temp, buf);
            badge_update(s_temp_badge, s_temp_badge_sym, s_temp_badge_val,
                         sensor_history_get_temp_trend(),
                         temp_delta_to_display(sensor_history_get_temp_delta()),
                         "\xc2\xb0");
            ...
            if (s.valid & SENSOR_CAP_HUMIDITY) {
                ...
                float dp = dew_point_calc(s.temperature, s.humidity);
                snprintf(buf, sizeof(buf), "%.1f", temp_to_display(dp));
            ...
            /* CO2 ppm — color-coded by air quality level. Value is the hub's
             * merged reading (real NDIR CO2 preferred over a BME680 estimate). */
            if (s_lbl_gas) {
            ...
            if (s.valid & SENSOR_CAP_PRESSURE) {
                snprintf(buf, sizeof(buf), "%.0f", s.pressure / 100.0);
...
void ui_main_create(void)
{
    ...
    bool sensor_present = (sensor_hub_caps() != 0);
    s_sensor_present = sensor_present;

    s_use_fahrenheit = false;
    nvs_settings_get_use_fahrenheit(&s_use_fahrenheit);
    const char *temp_unit_str = s_use_fahrenheit ? "\xc2\xb0\x46" /* °F */
                                                  : "\xc2\xb0\x43" /* °C */;
    ...
    if (sensor_present) {
        ...
        s_lbl_temp_unit = lv_label_create(temp_val);
        lv_label_set_text(s_lbl_temp_unit, temp_unit_str);
        lv_obj_set_style_text_font(s_lbl_temp_unit, &lv_font_montserrat_14, 0);
        lv_obj_set_style_text_color(s_lbl_temp_unit, C_TEMP, 0);
        lv_obj_set_style_text_opa(s_lbl_temp_unit, LV_OPA_70, 0);
        lv_obj_set_style_margin_bottom(s_lbl_temp_unit, 24, 0);
        ...
        s_lbl_dew_unit = lv_label_create(dew_val);
        lv_label_set_text(s_lbl_dew_unit, temp_unit_str);
        lv_obj_set_style_text_font(s_lbl_dew_unit, &lv_font_montserrat_14, 0);
        lv_obj_set_style_text_color(s_lbl_dew_unit, C_DEW, 0);
        lv_obj_set_style_text_opa(s_lbl_dew_unit, LV_OPA_70, 0);
        lv_obj_set_style_margin_bottom(s_lbl_dew_unit, 4, 0);
        ...
        /* ── Sensor error card (x=8, y=39, w=224, h=224) ─────────────── */
        lv_obj_t *err_card = make_card(8, 39, 224, 224);

        lv_obj_t *err_lbl = lv_label_create(err_card);
        lv_label_set_text(err_lbl, "Connect sensor\nand reboot\nthe device");
        lv_obj_set_style_text_align(err_lbl, LV_TEXT_ALIGN_CENTER, 0);
        lv_obj_set_style_text_font(err_lbl, &lv_font_montserrat_14, 0);
        lv_obj_set_style_text_color(err_lbl, C_TREND_UP, 0);
        lv_obj_set_width(err_lbl, 180);
    ...
    /* ── CO2 card (x=8, y=270, w=109, h=43) — SCD4x or BME680 only ──── */
    if (has_co2()) {
        ...
        /* Real NDIR CO2 (SCD4x) vs. a BME680 estimate — label accordingly.
         * A BME680 CO2 source always carries a gas-resistance capability, so
         * that is the static tell for an equivalent reading. */
        lv_label_set_text(lbl_gas_hdr,
                          sensor_hub_has(SENSOR_CAP_GAS_RESISTANCE) ? "CO2 EQ." : "CO2");
